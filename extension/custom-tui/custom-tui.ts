...
export default function customTuiExtension(pi: ExtensionAPI) {
	...
	// Agent Switcher owns routing. Never infer agent from system prompt text:
	// SYSTEM.md necessarily mentions every agent, which falsely selected Product Owner.
	function detectActiveAgent(_systemPrompt: string, _userPrompt?: string): string {
		return "None";
	}
	...
	pi.on("before_agent_start", (event, ctx) => {
		...
		// Do not derive routing from rendered system prompt. Agent Switcher updates
		// global.activeAgentName before this status render.
